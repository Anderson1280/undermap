...
Nenhum dado pessoal sensível é coletado. Os leads salvos localmente ficam exclusivamente no seu computador (`undermap.db`).

### Opt-out

Todo e-mail enviado pelo Undermap inclui obrigatoriamente a instrução:

> "Para não receber mais e-mails, responda 'remover' neste e-mail."

O remetente é responsável por honrar os pedidos de remoção.

### Secrets e credenciais

- Chaves de API e senhas ficam **exclusivamente no `.env` local**
- O `.env` está no `.gitignore` — nunca vai para o GitHub
- O repositório público não contém nenhuma credencial real

### Audit trail

Todos os leads encontrados, qualificados e e-mails enviados são registrados localmente com timestamp no banco SQLite — permitindo rastreabilidade completa das operações.

### Uso responsável

Esta ferramenta deve ser utilizada em conformidade com a **Lei Geral de Proteção de Dados (LGPD — Lei 13.709/2018)** e as políticas de uso do Google Maps Platform e da Receita Federal.
